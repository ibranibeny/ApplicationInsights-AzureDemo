...
// Stamp every telemetry item with this service's cloud role name so each ACI shows up
// as a distinct node on the Application Map (read from the SERVICE_ROLE_NAME env var).
builder.Services.AddSingleton<ITelemetryInitializer, CloudRoleNameInitializer>();
// HttpClient calls are auto-tracked as dependencies and propagate W3C trace context,
// which draws the edges between services on the Application Map.
...
// Distributed-tracing endpoint: does a little local work, then calls every downstream
// service listed in the DOWNSTREAM_SERVICES env var (comma-separated base URLs). Each
// hop is auto-captured as a dependency and the downstream request is correlated into the
// same operation, so the Application Map renders the full multi-service topology.
app.MapGet("/api/call", async (TelemetryClient telemetry, IHttpClientFactory httpFactory, int? depth) =>
{
    var role = Environment.GetEnvironmentVariable("SERVICE_ROLE_NAME") ?? "appi-demo-web";
    var remainingDepth = depth ?? 5;

    // Local "business" work.
    var sw = System.Diagnostics.Stopwatch.StartNew();
    await Task.Delay(Random.Shared.Next(20, 120));
    sw.Stop();
    telemetry.TrackDependency("InMemory", "LocalWork", DateTime.UtcNow.AddMilliseconds(-sw.ElapsedMilliseconds), sw.Elapsed, true);

    var downstreamResults = new List<object>();
    var downstream = Environment.GetEnvironmentVariable("DOWNSTREAM_SERVICES");
    if (remainingDepth > 0 && !string.IsNullOrWhiteSpace(downstream))
    {
        var client = httpFactory.CreateClient();
        client.Timeout = TimeSpan.FromSeconds(30);
        foreach (var baseUrl in downstream.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            try
            {
                var resp = await client.GetAsync($"{baseUrl.TrimEnd('/')}/api/call?depth={remainingDepth - 1}");
                var body = await resp.Content.ReadAsStringAsync();
                downstreamResults.Add(new { target = baseUrl, status = (int)resp.StatusCode, body });
            }
            catch (Exception ex)
            {
                telemetry.TrackException(ex);
                downstreamResults.Add(new { target = baseUrl, error = ex.Message });
            }
        }
    }

    telemetry.TrackEvent("ServiceCall", new Dictionary<string, string> { { "Role", role }, { "Depth", remainingDepth.ToString() } });
    return Results.Ok(new { service = role, depth = remainingDepth, downstream = downstreamResults });
});
...
// Sets cloud_RoleName for every telemetry item from the SERVICE_ROLE_NAME env var so each
// container instance appears as its own node on the Application Map.
public class CloudRoleNameInitializer : ITelemetryInitializer
{
    private readonly string _roleName =
        Environment.GetEnvironmentVariable("SERVICE_ROLE_NAME") ?? "appi-demo-web";
